...
class OneRandomXPerBatch(AcquisitionBase):
    """Acquisition from uniform distribution."""
    ...
    def acquire(self, n, t=None):
        ...
        """
        bounds = np.stack(self.model.bounds)
        x = ss.uniform(bounds[:, 0], bounds[:, 1] - bounds[:, 0])\
            .rvs(size=(1, self.model.input_dim), random_state=self.random_state) 
        return np.repeat(x, n, axis=0)


class QuantileAcquisition(AcquisitionBase):
    @property
    def delta(self):
        """Return the inverse of exploration rate."""
        return 1 / self.exploration_rate
    ...
    def evaluate(self, x, t=None):
        ...
        # return np.quantile(sampled_fs, 0.3, axis=0)
